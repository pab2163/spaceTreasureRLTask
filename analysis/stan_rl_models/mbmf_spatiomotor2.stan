...
transformed parameters {
  ...
  for (i in 1:N) {
      ...
      a3[i]     = Phi_approx( mu_pr[8] + sigma[8] * a3_pr[i] );
      lambda2[i] = Phi_approx( mu_pr[9] + sigma[9] * lambda2_pr[i] );
      w2[i]      = Phi_approx( mu_pr[10] + sigma[10] * w2_pr[i] );
  }
}
model {
  // Hyperparameters
  ...
  for (i in 1:N) {
    ...
    vector[6] v_hybrid;  // hybrid stimulus values for all 6 stimuli
    ...
    for (t in 1:Tsubj[i])  {
      ...
      // compute v_hybrid
      v_hybrid[1] = (1-w2[i]) * (w[i] * v_mb[1] + (1-w[i]) * v_mf[1]) + w2[i] * v_keys[1];   // hybrid stim 1= weighted sum
      v_hybrid[2] = (1-w2[i]) * (w[i] * v_mb[2] + (1-w[i]) * v_mf[2]) + w2[i] * v_keys[2];   // hybrid stim 2= weighted sum
      ...
      if (level2_choice[i,t] > 2) {  // level2_choice = 3 or 4
        v_hybrid[5] = (1-w2[i]) * v_mf[5] + w2[i] * v_keys[3];
        v_hybrid[6] = (1-w2[i]) * v_mf[6] + w2[i] * v_keys[4];
        level2_prob_choice2 = inv_logit( beta2[i]*( v_hybrid[6] - v_hybrid[5] ) );
      } else { // level2_choice = 1 or 2
        v_hybrid[3] = (1-w2[i]) * v_mf[3] + w2[i] * v_keys[3];
        v_hybrid[4] = (1-w2[i]) * v_mf[4] + w2[i] * v_keys[4];
        level2_prob_choice2 = inv_logit( beta2[i]*( v_hybrid[4] - v_hybrid[3] ) );
      }
...
generated quantities {
  ...
  mu_a3     = Phi_approx( mu_pr[8] );
  mu_lambda2 = Phi_approx( mu_pr[9] );
  mu_w2      = Phi_approx( mu_pr[10] );
  ...
  for (i in 1:N) {
    ...
    vector[6] v_mf;    // model-free stimulus values for level 1&2 (1,2--> level 1, 3-6--> level 2)
    vector[4] v_keys;
    vector[6] v_hybrid;  // hybrid stimulus values for level 1 (2 stimuli)
